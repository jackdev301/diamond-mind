...
cd frontend
NEXT_PUBLIC_API_URL="http://${LOCAL_IP}:8000" npm run dev -- --hostname 0.0.0.0 &
FRONTEND_PID=$!
cd "$SCRIPT_DIR"
...
echo ""
echo "  Backend:  http://${LOCAL_IP}:8000/docs"
echo "  Frontend: http://${LOCAL_IP}:3000  ← share with Arnav"
echo ""
